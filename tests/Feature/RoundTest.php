...
it('marks the top scoring submission as accepted', function () {
    ...
    $this->assertEquals($submission2->fresh()->status, 'rejected');
    $this->assertEquals($submission1->fresh()->status, 'accepted');
}); 
...
it('selects a random top-scoring submission if tied submissions also have tied aggregate_scores', function () {
    ...
    $this->assertEquals(Submission::where('status', 'accepted')->count(), 1);
}); 
